...
- **Clients must cryptographically verify**: connection access JWTs via JWKS (`jose` remote JWKS + `iss`/`aud`), and **must not** treat introspection alone as proof of possession unless the API is fixed to verify the token or bind proof (see [FC-SEC-003](./FC-SEC-003-introspection-token-not-verified)).
...
**Gap:** introspection path verifies the **caller’s** app signature but not the **connection token’s** signature ([FC-SEC-003](./FC-SEC-003-introspection-token-not-verified)).

## Findings table

| ID | Title | Severity | Area | CWE |
| --- | --- | --- | --- | --- |
| [FC-SEC-001](./FC-SEC-001-hardcoded-falcon-signing-key) | Hardcoded fallback Falcon Connect signing key | **CRITICAL** | api / crypto | CWE-798 |
| [FC-SEC-002](./FC-SEC-002-hardcoded-demo-app-keys) | Hardcoded demo trusted-app private keys in repo and DB bootstrap | **CRITICAL** | api / apps | CWE-798 |
| [FC-SEC-003](./FC-SEC-003-introspection-token-not-verified) | Introspection accepts `connectionToken` without cryptographic verification | **HIGH** | api / protocol | CWE-345 |
| [FC-SEC-004](./FC-SEC-004-sdk-introspection-fallback-decodeJwtUnsafe) | SDK introspection fallback uses `decodeJwtUnsafe` | **HIGH** | sdk | CWE-347 |
| [FC-SEC-005](./FC-SEC-005-signed-request-host-not-bound) | Request signing canonical string omits scheme and host | **MEDIUM** | sdk / api / protocol | CWE-444 |
| [FC-SEC-006](./FC-SEC-006-nonce-replay-store-growth) | Nonce replay table grows without TTL cleanup | **MEDIUM** | db / api | CWE-400 |
| [FC-SEC-007](./FC-SEC-007-dashboard-connection-status-idor) | Dashboard can update any connection by id | **HIGH** | api / auth | CWE-639 |
| [FC-SEC-008](./FC-SEC-008-ops-surface-data-exposure) | Ops ORPC procedures expose registry and activity to any logged-in user | **MEDIUM** | api / auth | CWE-200 |
| [FC-SEC-009](./FC-SEC-009-install-intent-token-in-url) | Install intent JWT passed in query string | **MEDIUM** | protocol / apps | CWE-598 |
| [FC-SEC-010](./FC-SEC-010-validation-error-detail-leakage) | Zod validation errors returned verbatim to clients | **LOW** | api | CWE-209 |
| [FC-SEC-011](./FC-SEC-011-demo-insecure-reference-patterns) | Demo apps embed secrets and unsafe token UX | **HIGH** | apps | CWE-798 |
| [FC-SEC-012](./FC-SEC-012-source-callback-query-unauthenticated) | Source callback trusts query parameters without binding | **MEDIUM** | apps / protocol | CWE-345 |
| [FC-SEC-013](./FC-SEC-013-no-rate-limit-on-v1) | No rate limiting on partner `/v1` API | **MEDIUM** | api / infra | CWE-400 |
...
8. Add edge or middleware rate limits for `/v1/*` ([FC-SEC-013](./FC-SEC-013-no-rate-limit-on-v1)).
