---
title: "FALCON Connect security review — full report (25 March 2025)"
description: Threat model, data flow, crypto invariants, findings table, residual risk, and backlog.
---

# Full security review report

This page continues from the [executive summary](/docs/reviews/security/25-03-2025). Findings are grouped by severity in the sidebar (**Critical** → **Low**).
...
**Gap:** introspection path verifies the **caller’s** app signature but not the **connection token’s** signature ([FC-SEC-003](/docs/reviews/security/25-03-2025/high/FC-SEC-003-introspection-token-not-verified)).

## Findings table

Rows are ordered **Critical → High → Medium → Low**.

| ID | Title | Severity | Area | CWE |
| --- | --- | --- | --- | --- |
| [FC-SEC-001](/docs/reviews/security/25-03-2025/critical/FC-SEC-001-hardcoded-falcon-signing-key) | Hardcoded fallback Falcon Connect signing key | **CRITICAL** | api / crypto | CWE-798 |
| [FC-SEC-002](/docs/reviews/security/25-03-2025/critical/FC-SEC-002-hardcoded-demo-app-keys) | Hardcoded demo trusted-app private keys in repo and DB bootstrap | **CRITICAL** | api / apps | CWE-798 |
| [FC-SEC-003](/docs/reviews/security/25-03-2025/high/FC-SEC-003-introspection-token-not-verified) | Introspection accepts `connectionToken` without cryptographic verification | **HIGH** | api / protocol | CWE-345 |
| [FC-SEC-004](/docs/reviews/security/25-03-2025/high/FC-SEC-004-sdk-introspection-fallback-decodeJwtUnsafe) | SDK introspection fallback uses `decodeJwtUnsafe` | **HIGH** | sdk | CWE-347 |
| [FC-SEC-007](/docs/reviews/security/25-03-2025/high/FC-SEC-007-dashboard-connection-status-idor) | Dashboard can update any connection by id | **HIGH** | api / auth | CWE-639 |
| [FC-SEC-011](/docs/reviews/security/25-03-2025/high/FC-SEC-011-demo-insecure-reference-patterns) | Demo apps embed secrets and unsafe token UX | **HIGH** | apps | CWE-798 |
| [FC-SEC-005](/docs/reviews/security/25-03-2025/medium/FC-SEC-005-signed-request-host-not-bound) | Request signing canonical string omits scheme and host | **MEDIUM** | sdk / api / protocol | CWE-444 |
| [FC-SEC-006](/docs/reviews/security/25-03-2025/medium/FC-SEC-006-nonce-replay-store-growth) | Nonce replay table grows without TTL cleanup | **MEDIUM** | db / api | CWE-400 |
| [FC-SEC-008](/docs/reviews/security/25-03-2025/medium/FC-SEC-008-ops-surface-data-exposure) | Ops ORPC procedures expose registry and activity to any logged-in user | **MEDIUM** | api / auth | CWE-200 |
| [FC-SEC-009](/docs/reviews/security/25-03-2025/medium/FC-SEC-009-install-intent-token-in-url) | Install intent JWT passed in query string | **MEDIUM** | protocol / apps | CWE-598 |
| [FC-SEC-012](/docs/reviews/security/25-03-2025/medium/FC-SEC-012-source-callback-query-unauthenticated) | Source callback trusts query parameters without binding | **MEDIUM** | apps / protocol | CWE-345 |
| [FC-SEC-013](/docs/reviews/security/25-03-2025/medium/FC-SEC-013-no-rate-limit-on-v1) | No rate limiting on partner `/v1` API | **MEDIUM** | api / infra | CWE-400 |
| [FC-SEC-010](/docs/reviews/security/25-03-2025/low/FC-SEC-010-validation-error-detail-leakage) | Zod validation errors returned verbatim to clients | **LOW** | api | CWE-209 |
...
8. Add edge or middleware rate limits for `/v1/*` ([FC-SEC-013](/docs/reviews/security/25-03-2025/medium/FC-SEC-013-no-rate-limit-on-v1)).
...
Each finding has its own page under **Critical**, **High**, **Medium**, or **Low** in the sidebar, with severity, confidence, impact, evidence, and mitigations.
